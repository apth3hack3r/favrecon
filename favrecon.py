...
st.write("Enter a valid favicon url:")
url=st.text_input("(A valid url here should start with http or https)")
...
if st.button('Go'):
	try:
		...
		if favicon_hash!=None:
			with st.expander(" ",expanded=True):
				
				st.text("Favicon Hash:")
				st.code(favicon_hash)
				st.text("Product/Application:")
				if favicon_hash in services.keys():
					st.code(services[favicon_hash])
				else:
					st.code("Not Found")

				st.text("Shodan Query:")
				st.write("[https://www.shodan.io/search?query=http.favicon.hash:{}](https://www.shodan.io/search?query=http.favicon.hash:{})".format(favicon_hash,favicon_hash))

				st.write("\n\n\nNOTE:")
				st.write("If the above shodan query didn't yeild any results you may as well try your luck with below query.")
				st.write("[https://www.shodan.io/search?query=http.html_hash:{}](https://www.shodan.io/search?query=http.html_hash:{})".format(favicon_hash,favicon_hash))
				
	except:
		st.error("Please Enter a valid URL")

st.write("Created By: [apth3hack3r](https://github.com/apth3hack3r/favrecon)")